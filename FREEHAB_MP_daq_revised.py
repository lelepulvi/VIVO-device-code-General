...
def record_daq(Allexit, record_start_fd, c_time, LogFile, Lr, Ll, psRu, psRd, psLu,psLd, FSRrh, FSRrt, FSRlh, FSRlt ):      # varaibles need to change
...
## Execute Multiple Process ############################################################
    Daq = mp.Process(target=record_daq, args=(Allexit, record_start_fd, c_time, LogFile, Lr, Ll, psRu, psRd, psLu,psLd, FSRrh, FSRrt, FSRlh, FSRlt ))
    ...
    Daq.start()   
    print("DAQ record active:", Daq.pid, "alive:", Daq.is_alive(), flush=True )
    ...
    try:      
        ...
        while True:
            ...
            ## Calculate Threshold #################################################################################
            # print(Lr, Ll, psRu, psRd, psLu, psLd)
                # print(Analogs)
