...
# --- process_leg updates actuator1.value and actuator2.value (side-effect) ---
    ...
    def make_turn_detector(
        ...
        def update(yaw_val, t=None, yaw_rate_dps=None):
            ...
            """
            Rate-only turn gate:
          - ENTER rotating  if |yaw_rate| >= omega_on_dps for >= min_enter
          - EXIT  rotating  if |yaw_rate| <= omega_off_dps for >= min_exit
            No angle-span / 180° completion logic.
            """
            nonlocal state, last_yaw_raw, yaw_rate_ema
            nonlocal last_t, enter_t, exit_t
            #nonlocal enter_heading, yaw_unwrapped
            ...
            # ---------------- rate-only state machine ----------------
            if state == 0:  # NOT rotating
                if ar >= omega_on_dps:
                    if enter_t is None:
                        enter_t = t
                    elif (t - enter_t) >= min_enter:
                        state = 1  # confirm: now ROTATING
                        enter_t = None
                        exit_t = None
                else:
                    enter_t = None
                ...
                if ar <= omega_off_dps:
                    if exit_t is None:
                        ...
                        state = 0  # confirm: back to NOT ROTATING
                        exit_t = None
                else:
                    exit_t = None

            # Keep 'yaw_span_deg' for backward-compat with your debug prints
            return state, {"t": t, "yaw_span_deg": 0.0, "yaw_rate_dps": yaw_rate}
