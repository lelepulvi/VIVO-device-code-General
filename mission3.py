...
def record_daq(Allexit, record_start_fd, c_time, LogFile, Lr, Ll, psRu, psRd, psLu,psLd, FSRrh, FSRrt, FSRlh, FSRlt, switch, heelR,heelL,HeelRp, HeelLp,toeR,toeL, supportTimeR, unsupportTimeR, supportTimeL, unsupportTimeL):      # varaibles need to change
    ...
    taskAI = nidaqmx.Task()
    taskAI.ai_channels.add_ai_voltage_chan("Dev1/ai0", min_val=0, max_val= 5)    # Loadcell right
    taskAI.ai_channels.add_ai_voltage_chan("Dev1/ai1", min_val=0, max_val= 5)    # Loadcell left
    taskAI.ai_channels.add_ai_voltage_chan("Dev1/ai2", min_val=0, max_val= 5)    # Pressure sensor right up
    taskAI.ai_channels.add_ai_voltage_chan("Dev1/ai3", min_val=0, max_val= 5)    # Pressure sensor right down
    taskAI.ai_channels.add_ai_voltage_chan("Dev1/ai4", min_val=0, max_val= 5)    # Pressure sensor left up
    
    taskAI.ai_channels.add_ai_voltage_chan("Dev1/ai5", min_val=0, max_val= 5)    # Pressure sensor left down
    
    
    
    taskAI.ai_channels.add_ai_voltage_chan("Dev1/ai6", min_val=0, max_val= 5)    # FSR left heel
    taskAI.ai_channels.add_ai_voltage_chan("Dev1/ai7", min_val=0, max_val= 5)    # FSR left toe
    taskAI.ai_channels.add_ai_voltage_chan("Dev1/ai16", min_val=0, max_val= 5)   # FSR left heel
    taskAI.ai_channels.add_ai_voltage_chan("Dev1/ai17", min_val=0, max_val= 5)   # FSR left toe
...
def Pressure_foot(Allexit,  FSRrh, FSRrt, FSRlh, FSRlt):
    ...
    Pressure_serialL = serial.Serial('COM22', 460800,    # left 
    ...
    time.sleep(2)
    Pressure_serialR = serial.Serial('COM23', 460800,    # left 
             parity=serial.PARITY_NONE,
             stopbits=serial.STOPBITS_ONE,
             bytesize=serial.EIGHTBITS, 
             timeout=.1)
...
## Execute Multiple Process ############################################################
    ...
    try:      
        ...
        print("\nRHeel- Toe - LHeel - Toe - RThigh -rSHANK-LThigh-Shank- Hip   Yaw  Heading  State Pressure", flush=True)
        ...
        while True:
            ...
            IMU_angle = copy.copy(IMU_5z.value)  # from shared variable
            ...
            result = state_detection(IMU_angle, last_angle, unwrapped_angle,
                                    ...
                                    last_stable_angle=last_stable_angle,swaylimit=30, stable_time=0.8,
           ...
           # print(f"State={state}, Pressure={pressure}, err={err_dbg:.1f} Yaw={raw_yaw:.2f}")
            ...
            if keyboard.is_pressed('r'):                
                ...
                if firsttime == 1:
                    ...
                    print("RHeel- Toe - LHeel - Toe - RThigh -rSHANK-LThigh-Shank- Hip   Yaw  Heading  State Pressure")
                record_start  = 1
          ...
          #  sys.stdout.write('\r' + ' ' * 80 + '\r')  # Clear the line
           # print('  {:d},   {:d},      {:d},     {:d},   {:3.2f},  {:3.2f}, {:3.2f}, {:3.2f}, {:3.2f}, {:4.1f}, {:4.2f},    {:d},   {:.2f}'.format(int(heelR.value),  int(toeR.value), int(heelL.value),  int(toeL.value),float(IMU_1),  float(IMU_2z.value), float(IMU_3z.value),  float(IMU_4z.value), float(IMU_5z.value), unwrapped, heading_anchor, state, pressure), end ='', flush=True ) 
          
            sys.stdout.write('\r  {:d},   {:d},      {:d},     {:d},   {:3.2f},  {:3.2f}, {:3.2f}, {:3.2f}, {:3.2f}, {:4.1f}, {:4.2f},    {:d},   {:.2f}'.format(
    int(heelR.value), int(toeR.value), int(heelL.value), int(toeL.value),
    float(IMU_1), float(IMU_2), float(IMU_3), float(IMU_4), float(IMU_5z.value),
    unwrapped_angle, heading_anchor, state, pressure
       ))
            sys.stdout.flush()          
           ...
           # print("\033[K", end='')  # Clear to the end of line
           # print(f"h= {heelR.value}, h= {toeR.value},h= {heelL.value},h= {toeL.value},h={IMU_1z.value:.3f},h={IMU_2z.value:.3f},h={IMU_3z.value:.3f},h= {IMU_4z.value:.3f},h= {IMU_5z.value:.3f}, Yaw={unwrapped:4.1f}, heading={heading_anchor:4.2f}, State={state}, Pressure={pressure:.2f}", end ='\r', flush=True) 
           # print(f"hr={heelR.value},tr={toeR.value},hl{heelL.value},tr={toeL.value},rt={IMU_1z.value:.3f},rs={IMU_2z.value:.3f},lt={IMU_3z.value:.3f},ls={IMU_4z.value:.3f},h={IMU_5z.value:.3f}, Yaw={unwrapped:4.1f}, heading={heading_anchor:4.2f}, State={state}, Pressure={pressure:.2f}", end ='\r', flush=True) 
           # print(f"hr={heelR.value},  tr={toeR.value}, hl={heeleL.value}, tr={toeL.value}, rt={IMU_1z.value:.3f}, rs={IMU_2z.value:.3f}, lt={IMU_3z.value:.3f}, ls={IMU_4z.value:.3f}, h={IMU_5z.value:.3f}, Yaw={unwrapped:4.1f}, heading={heading_anchor:4.2f}, State={state}, Pressure={pressure:.2f}", end ='\r', flush=True) 
...
################################################################
    ...
    # cwd = os.getcwd() 
        ...
        base_dir = r"C:\Users\ep15603\OneDrive - University of Bristol\Desktop\VIVO hub\3. STS-Walking device\VIVO-device-code-General-main\VIVO-device-code-General-main" # r"C:\Users\jf20027\Desktop\LocalDir"
