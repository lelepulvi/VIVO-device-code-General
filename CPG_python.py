...
# --- process_leg updates actuator1.value and actuator2.value (side-effect) ---
    def process_leg(heel_value, last_heel, gaittime, supportTime, unsupportTime,
        ...
        if   unsupportTime.value == 0 or  supportTime.value == 0:
           
            actuator1.value = 0        # was level
            actuator2.value = 0        # was level
            #print("here")
        elif  currenttime >= supportTime.value and currenttime <= unsupportTime.value:
           
            actuator1.value = level        # was level
            actuator2.value = level         # was level
            #print("here")
        else:
            actuator1.value = 0.0    # was 0
            actuator2.value = 0.0    # was 0
           # print("not here")
    ...
    def update_heel_strike_phases(current_time):
        ...
        # ========================================================
        # Right heel-strike rising edge
        # ========================================================

        if heelRight == 1 and previousHeelRight == 0:
            ...
            # Reset right target and right exosuit phases
            theta_tr = 0.0
            theta_er = 0.0

        # ========================================================
        # Left heel-strike rising edge
        # ========================================================

        if heelLeft == 1 and previousHeelLeft == 0:
            ...
            # Reset left target and left exosuit phases
            theta_tl = 0.0
            theta_el = 0.0
        ...
        # ========================================================
        # Right elapsed time and phase
        # ========================================================

        if rightHeelStrikeTime is not None:

            time_since_right_heel_strike = current_time- rightHeelStrikeTime
            

            if time_since_right_heel_strike < 0.0:
                time_since_right_heel_strike = 0.0

            if rGaitTime > 0.0:

                phase_right = 2.0* math.pi* time_since_right_heel_strike/ rGaitTime
                

                phase_right = wrap_to_2pi(phase_right)
        ...
        # ========================================================
        # Left elapsed time and phase
        # ========================================================

        if leftHeelStrikeTime is not None:

            time_since_left_heel_strike = current_time - leftHeelStrikeTime
            

            if time_since_left_heel_strike < 0.0:
                time_since_left_heel_strike = 0.0

            if lGaitTime > 0.0:

                phase_left = 2.0* math.pi * time_since_left_heel_strike/ lGaitTime
                

                phase_left = wrap_to_2pi(phase_left)
    ...
    def PID(error, prev_error):
        ...
        derivative = dDerivative_dt(error, prev_error)
    ...
    def dz_dt(time_seconds,y, z,goal,error,theta_i,side):
        ...
        if side == 1:
            ...
            R = math.fmod(lookup_right(theta_i),6.28)

        elif side == 2:

            R = math.fmod(lookup_left(theta_i), 6.28)
    ...
    def exosuit():
        ...
        exosuit_torque_right = (RampValue * exosuit_torque_right)

        exosuit_torque_left = (RampValue * exosuit_torque_left)
    ...
    def CPG():
        ...
        y_tr = transformational_terms(goal_function_right(), 0.0, phase_right, y_tr, 1)

        # Right exosuit compensation
        if rGaitTime > 0.0:
            tau = ( rGaitTime/ (2.0 * math.pi))

        y_er = transformational_terms( 0.0, error_right, phase_right, y_er, 1)

        # Left target trajectory
        if lGaitTime > 0.0:
            tau = (lGaitTime / (2.0 * math.pi))

        y_tl = transformational_terms(goal_function_left(), 0.0, phase_left, y_tl, 2)

        # Left exosuit compensation
        if lGaitTime > 0.0:
            tau = (lGaitTime / (2.0 * math.pi))

        y_el = transformational_terms(0.0,error_left,phase_left, y_el,2)
    ...
    def update_cpg_controller(
        ...
        if last_time_seconds is None:
            ...
            dt = (current_time_seconds- last_time_seconds)
        ...
        if HeelRp > 0.0:
            ...
            rGaitTime = float(HeelRp)

        if HeelLp > 0.0:

            lGaitTime = float(HeelLp)
        ...
        requiredPressure_R = pressureFromTorque(exosuit_torque_right)

        requiredPressure_L = pressureFromTorque(exosuit_torque_left)
        ...
        state.lookup_right = lookup_right(phase_right)
        state.lookup_left = lookup_left(phase_left)
