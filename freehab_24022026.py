...
def update_unwrap(IMU_angle):
    global last_raw, unwrapped_angle

    if last_raw is None:
        last_raw = IMU_angle
        unwrapped_angle = IMU_angle
        return unwrapped_angle

    # difference from previous RAW measurement
    ...
    # accumulate
    unwrapped_angle += dtheta

    # 🔴 IMPORTANT: update last_raw every step
    last_raw = IMU_angle
    ...
    while not quit:
        ...
        if zenEvent.event_type == openzen.ZenEventType.ImuData:
         if   zenEvent.sensor == imu1.sensor and \
            ...
            IMU_1 = data
            #IMU_11.value = data
            IMU_11.value = update_unwrap(IMU_1)
        ...
        # check for IMU2 event
            ...
            IMU_2 = data
           # IMU_22.value = data
            IMU_22.value = update_unwrap(IMU_2)
            ...
            lt = data
            #IMU_33.value = data
            IMU_33.value = update_unwrap(IMU_3)
           ...
           # data3 = [ts - t3now] + [perf_counter()- detected_time] + [data] 
            ...
            ls = data
           # IMU_44.value = data
            IMU_44.value = update_unwrap(IMU_4)
            ...
            hip = data
            #IMU_55.value = data
            IMU_55.value = update_unwrap(IMU_5)
...
def Walk(heelR,heelL,toeR,toeL,Allexit, Rallow, Lallow, supportTimeR, unsupportTimeR, supportTimeL, unsupportTimeL, supportTimeRAn, unsupportTimeRAn, supportTimeLAn,unsupportTimeLAn, R, L, RR, LL, Emg_sig, r_pressed, IMU_5z, Applied_pressure):
    ...
    # lab COM3, home COM17
    arduino = serial.Serial('COM17', 115200, parity=serial.PARITY_NONE,
...
def record_daq(Allexit, record_start_fd, c_time, LogFile, Lr, Ll, psRu, psRd, psLu,psLd, FSRrh, FSRrt, FSRlh, FSRlt, switch, heelR,heelL,HeelRp, HeelLp,toeR,toeL, supportTimeR, unsupportTimeR, supportTimeL, unsupportTimeL, IMU_11, IMU_22, IMU_33, IMU_44, IMU_55, IMU_1z, IMU_2z, IMU_3z, IMU_4z, IMU_5z):      # varaibles need to change
    ...
    taskAI.start()   
    fa=open(LogFile,'w')
    data = ["Time"]  + ["R Loadcell"] + ["L Loadcell"]  + ["Pressure R BAM UP"] + ["Pressure R BAM Down"] + ["Pressure L BAM UP"] + ["Pressure L BAM Down"] + ["FSR R Heel"] + ["FSR R Toe"] + ["FSR L Heel"] + ["FSR L Toe"] + ["heelR"] + ["heelL"] +["HeelRp"] + ["HeelLp"] +["toeR"]+["toeL"]+ ["supportTimeR"] + ["unsupportTimeR"]+ ["supportTimeL"]+ ["unsupportTimeL"] + ["R thigh"] + ["R shank"] + ["L thigh"] + ["L shank"] +  ["R knee"] + ["L knee"] + ["hip x"] + ["hip r"]
    ...
    while not Allexit.value:
        ...
        if record_start_fd.value == 1:                      
           # if perf_counter() - record_interval > 0.009:        
                # print("here")
                dataA = [perf_counter() - c_time.value] +  [Lr.value] +  [Ll.value]  + [psRu.value] + [psRd.value] + [psLu.value] + [psLd.value] + [FSRrh.value] + [FSRrt.value] + [FSRlh.value] + [FSRlt.value]  + [heelR.value] + [heelL.value] +[HeelRp.value] + [HeelLp.value] +[toeR.value]+[toeL.value]+ [supportTimeR.value] + [unsupportTimeR.value]+ [supportTimeL.value]+ [unsupportTimeL.value] + [IMU_11.value] + [IMU_22.value] + [IMU_33.value] + [IMU_44.value] + [IMU_22.value - IMU_11.value] + [IMU_44.value - IMU_33.value] + [IMU_55.value] + [IMU_5z.value]
                for ele in dataA:
                    fa.write(str(ele)+',')
                fa.write('\n')
...
def Pressure_foot(Allexit,  FSRrh, FSRrt, FSRlh, FSRlt):
    ...
    Pressure_serialL = serial.Serial('COM16', 460800,    # left 
    ...
    time.sleep(2)
    Pressure_serialR = serial.Serial('COM15', 460800,    # left 
             parity=serial.PARITY_NONE,
             stopbits=serial.STOPBITS_ONE,
             bytesize=serial.EIGHTBITS, 
             timeout=.1)
...
if __name__ == "__main__":
    ...
    rStrength = mp.Value('d', 0)    
    lStrength = mp.Value('d', 0)
    rLength = mp.Value('d', 0)    
    lLength = mp.Value('d', 0)
...
## Execute Multiple Process ############################################################
    Daq = mp.Process(target=record_daq, args=(Allexit, record_start_fd, c_time, LogFile, Lr, Ll, psRu, psRd, psLu,psLd, FSRrh, FSRrt, FSRlh, FSRlt,switch,heelR,heelL, HeelRp, HeelLp,toeR,toeL, supportTimeR, unsupportTimeR, supportTimeL, unsupportTimeL, IMU_11, IMU_22, IMU_33, IMU_44, IMU_55, IMU_1z, IMU_2z, IMU_3z, IMU_4z, IMU_5z ))
        ...
        while True:
          ...
          #  print('     {:d},        {:d},       {:d},       {:d},      {:.3f},     {:.3f},    {:.3f},    {:.3f}, {:.3f}'.format(int(heelR.value),  int(toeR.value), int(heelL.value),  int(toeL.value),float(IMU_1),  float(IMU_2), float(IMU_3),  float(IMU_4), float(IMU_5)), end ='\r' ) 
           # print('     {:d},        {:d},       {:d},       {:d},      {:.3f},     {:.3f},    {:.3f},    {:.3f}, {:.3f}, {:.3f}'.format(int(heelR.value),  int(toeR.value), int(heelL.value),  int(toeL.value),float(IMU_1z.value),  float(IMU_2z.value), float(IMU_3z.value),  float(IMU_4z.value), float(IMU_5z.value), float(pressure)), end ='\r' ) 
            print('     {:d},        {:d},       {:d},       {:d},      {:.3f},     {:.3f},    {:.3f},    {:.3f}, {:.3f}, {:.3f}'.format(int(heelR.value),  int(toeR.value), int(heelL.value),  int(toeL.value),float(IMU_11.value),  float(IMU_22.value), float(IMU_33.value),  float(IMU_44.value), float(IMU_5z.value), float(pressure)), end ='\r' ) 
         ...
         #   print('     {:.3f}, {:.3f}'.format( float(IMU_5z.value), float(THETA)), end ='\r' ) 
          #  print('            
            ...
            if keyboard.is_pressed('e') and r_pressed.value == 0:
                                ...
                                mlocal = 1
                                break
            if keyboard.is_pressed('u') and r_pressed.value == 0: 
                print("\nSit in neutral posture. Press ENTER to capture or 'n' to skip:")
                k = input()

                if k.lower() != 'n':
                    rSitThigh.value = IMU_11.value
                    rSitShank.value = IMU_22.value
                    lSitThigh.value = IMU_33.value
                    lSitShank.value = IMU_44.value
                    hipSit.value    = IMU_55.value

                    print("Sit posture captured.")
                    print(f"  R thigh : {rSitThigh.value:.2f}")
                    print(f"  R shank : {rSitShank.value:.2f}")
                    print(f"  L thigh : {lSitThigh.value:.2f}")
                    print(f"  L shank : {lSitShank.value:.2f}")
                    print(f"  Hip     : {hipSit.value:.2f}")
                else:
                    print("Sit posture skipped.")
                      
                print("\nStand upright. Press ENTER to capture or 'n' to skip:")
                k = input()

                if k.lower() != 'n':
                    rStandThigh.value = IMU_11.value
                    rStandShank.value = IMU_22.value
                    lStandThigh.value = IMU_33.value
                    lStandShank.value = IMU_44.value
                    hipStand.value    = IMU_55.value

                    print("Stand posture captured.")
                    print(f"  R thigh : {rStandThigh.value:.2f}")
                    print(f"  R shank : {rStandShank.value:.2f}")
                    print(f"  L thigh : {lStandThigh.value:.2f}")
                    print(f"  L shank : {lStandShank.value:.2f}")
                    print(f"  Hip     : {hipStand.value:.2f}")
                else:
                    print("Stand posture skipped.")
                def ask_float(prompt):
                    v = input(prompt + " (or 'n' to skip): ")
                    if v.lower() == 'n' or v.strip() == "":
                        return None
                    try:
                        return float(v)
                    except:
                ...
                print("\nEnter strength and leg length (optional)")

                v = ask_float("Right leg strength")
                if v is not None: rStrength.value = v

                v = ask_float("Left leg strength")
                if v is not None: lStrength.value = v

                v = ask_float("Right leg length")
                if v is not None: rLength.value = v

                v = ask_float("Left leg length")
                if v is not None: lLength.value = v
                
                print("\n--- Final Strength & Length Values ---")

                print(f"Right leg strength : {rStrength.value:.2f}")
                print(f"Left  leg strength : {lStrength.value:.2f}")

                print(f"Right leg length   : {rLength.value:.2f}")
                print(f"Left  leg length   : {lLength.value:.2f}")
